...
class Converter {
  ...
  convert(cf, alpha = 0, cbk=false) {
    ...
    if (this.cf === 'raw' || this.cf === 'raw_alpha' || this.cf === 'raw_chroma') {
      this.dOut = fs.readFileSync(this.path);
      return;
    }
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          if (!palette.has(colorKey)) {
            if (paletteIndex < paletteSize) {
              ...
              this.dOut.push(b, g, r, a);
              paletteIndex++;
            } else {
              palette.set(colorKey, paletteIndex - 1);
            }
          }

          let index = palette.get(colorKey);
          this.dOut.push(index);
          if(cbk){
            cbk()
          }
        }
        ...
        for (let y = 0; y < this.h; y++) {
          ...
          for (let x = 0; x < this.w; x++) {
            ...
            if (this.cf === 'true_color_332') {
              let c8 = (this.rAct & 0xE0) | ((this.gAct >> 3) & 0x1C) | ((this.bAct >> 6) & 0x03);
              this.dOut.push(c8);
              if (this.alpha) this.dOut.push(a);
            } else if (this.cf === 'true_color_565') {
              let c16 = ((this.rAct & 0xF8) << 8) | ((this.gAct & 0xFC) << 3) | ((this.bAct & 0xF8) >> 3);
              this.dOut.push(c16 & 0xFF);
              this.dOut.push((c16 >> 8) & 0xFF);
              if (this.alpha) this.dOut.push(a);
...
const lv_img_dsc_t ${this.out_name} = {
  ...
  convPx(x, y) {
    ...
    if (this.alpha) {
      a = 255 - ((a & 0xff000000) >> 23); // Alpha is 7 bit
